...
data = Dataset(dir_path + '/' +
               'static/data/S5P_NRTI_L2__NO2____20210420T064803_20210420T065303_18229_01_010400_20210420T073613.nc')
print(data)
...
def find_closest_lat_lon(data_list, data_find):
    if not len(data_list):
        return {"lat": np.nan, "lon": np.nan}
    try:
        return min(data_list, key=lambda p: dist_between_two_lat_lon(data_find['lat'],data_find['lon'],p['lat'],p['lon']))
    except TypeError:
        print('Not a list or not a number.')

def find_no2(lat_val,lon_val):
    print("=====>", float(lat_val), float(lon_val))
    select_lat = find_nearest(lat, float(lat_val))
    select_lon = find_nearest(lon, float(lon_val))
    ...
    location_to_find = {'lat': float(lat_val), 'lon': float(lon_val)}
    nearest_location =  find_closest_lat_lon(location_sel_list, location_to_find)
    if np.isnan(nearest_location["lat"]):
        return np.nan
