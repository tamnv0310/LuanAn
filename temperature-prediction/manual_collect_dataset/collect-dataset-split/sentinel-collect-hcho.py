...
# Set interval
f = open('air-quality-dataset/log/HCHO-log.txt', 'r')
query_date = f.read()
while True:
  ...
  np.savetxt('air-quality-dataset/HCHO/HCHO-'+query_date +
              '.csv', response_data, delimiter=',')

  log_file = open('air-quality-dataset/log/HCHO-log.txt', 'w')
  n = log_file.write(query_date)
  log_file.close()
  
  print("===Saved dataset " + 'HCHO-'+query_date + '.csv')
