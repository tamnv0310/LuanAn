#INSERT HCHO to Database in the dataset
...
for dist in dist_list:
    ...
    for doc in db.hcho.find({'dist_id': dist}).sort([('_id', DESCENDING)]).limit(1):
    ...
    for index, row in dist_data.iterrows():
        ...
        _val = dist_data['hcho'][index]
        if db.hcho.count_documents({'date': _date, 'dist_id': dist}) > 0:
            print('Exist one hcho {0}_{1} in Database'.format(dist, _date))
            continue
        else:
            ...
            #Step 3: Insert hcho object directly into MongoDB via isnert_one
            result = db.hcho.insert_one(_hcho)
            # Step 4: Print to the console the ObjectID of the new document
            print('Created one hcho {0}_{1} as {2}'.format(dist, _date, result.inserted_id,np.float64(_val)))
