#INSERT CO to Database in the dataset
...
for dist in dist_list:
    ...
    for doc in db.co.find({'dist_id': dist}).sort([('_id', DESCENDING)]).limit(1):
    ...
    for index, row in dist_data.iterrows():
        ...
        _val = dist_data['co'][index]
        if db.co.count_documents({'date': _date, 'dist_id': dist}) > 0:
            print('Exist one co {0}_{1} in Database'.format(dist, _date))
            continue
        else:
            ...
            #Step 3: Insert co object directly into MongoDB via isnert_one
            result = db.co.insert_one(_co)
            # Step 4: Print to the console the ObjectID of the new document
            print('Created one co {0}_{1} as {2}'.format(dist, _date, result.inserted_id,np.float64(_val)))
