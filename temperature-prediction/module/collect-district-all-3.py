...
col_head_list = {"1": "district", "2": "lastupdate", "3": "avg", "4": "pollutant"}
...
path_to_dataset = 'air-quality-dataset/_DISTRICT-ALL/hcm-3.csv'
...
f = open('air-quality-dataset/log/district-all-log-3.txt', 'r')
query_date = f.read()
...
while True:
    ...
    for dist in dist_list:
        ...
        # dist, ngay, gia tri tb, loai chat
        datarow = [dist, query_date, 0, ""]    
        ...
        if os.path.isfile(path_to_csv_no2):
            datarow[2]= getVal(data_no2, dist_list[dist][2], dist_list[dist][3])
            datarow[3] = "NO2"
            # Them vao cuoi file
            appendRow(path_to_dataset, datarow)
        else:
        ...
        if os.path.isfile(path_to_csv_o3):
            datarow[2]= getVal(data_o3, dist_list[dist][2], dist_list[dist][3])
            datarow[3] = "O3"
            # Them vao cuoi file
            appendRow(path_to_dataset, datarow)
        else:
        ...
        if os.path.isfile(path_to_csv_so2):
            datarow[2]= getVal(data_so2, dist_list[dist][2], dist_list[dist][3])
            datarow[3] = "SO2"
            # Them vao cuoi file
            appendRow(path_to_dataset, datarow)
        else:
        ...
        if os.path.isfile(path_to_csv_ch4):
            datarow[2]= getVal(data_ch4, dist_list[dist][2], dist_list[dist][3])
            datarow[3] = "CH4"
            # Them vao cuoi file
            appendRow(path_to_dataset, datarow)
        else:
        ...
        if os.path.isfile(path_to_csv_hcho):
            datarow[2]= getVal(data_hcho, dist_list[dist][2], dist_list[dist][3])
            datarow[3] = "HCHO"
            # Them vao cuoi file
            appendRow(path_to_dataset, datarow)
        else:
    ...
    log_file = open('air-quality-dataset/log/district-all-log-3.txt', 'w')
