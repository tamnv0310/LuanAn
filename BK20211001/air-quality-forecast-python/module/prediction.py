...
def predict_from_val(product=None, input=0):
    # load the model
    reconstructed_model = keras.models.load_model(dir_path+ '/models-py/rnn-lstm-'+product+'', custom_objects=None, compile=True, options=None)

    # load the scaler: with fit_transformed by the dataset
    scaler = load(open(dir_path+ '/models-py/rnn-lstm-'+product+'/scaler.pkl', 'rb'))
    ...
    print("Prediction "+product+" after inverse transform (micro mol/m2): ", pred)
    return pred