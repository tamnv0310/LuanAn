#INSERT NO2 to Database in the dataset
...
for dist in dist_list:
    ...
    for doc in db.no2.find({'dist_id': dist}).sort([('_id', DESCENDING)]).limit(1):
    ...
    for index, row in dist_data.iterrows():
        ...
        _val = dist_data['no2'][index]
        if db.no2.count_documents({'date': _date, 'dist_id': dist}) > 0:
            print('Exist one no2 {0}_{1} in Database'.format(dist, _date))
            continue
        else:
            ...
            #Step 3: Insert no2 object directly into MongoDB via isnert_one
            result = db.no2.insert_one(_no2)
