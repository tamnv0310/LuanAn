#INSERT SO2 to Database in the dataset
...
for dist in dist_list:
    ...
    for doc in db.so2.find({'dist_id': dist}).sort([('_id', DESCENDING)]).limit(1):
    ...
    for index, row in dist_data.iterrows():
        ...
        _val = dist_data['so2'][index]
        if db.so2.count_documents({'date': _date, 'dist_id': dist}) > 0:
            print('Exist one so2 {0}_{1} in Database'.format(dist, _date))
            continue
        else:
            ...
            #Step 3: Insert so2 object directly into MongoDB via isnert_one
            result = db.so2.insert_one(_so2)
            # Step 4: Print to the console the ObjectID of the new document
            print('Created one no2 {0}_{1} = {2} as id {3}'.format(dist, _date, result.inserted_id, np.float64(_val)))
